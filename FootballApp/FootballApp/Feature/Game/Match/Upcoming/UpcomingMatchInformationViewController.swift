...
class UpcomingMatchInformationViewController: UIViewController {
    ...
    private func setupViewControllers() {
        ...
        viewControllers = [matchSummaryVC, headToheadVC]
    }
...
extension UpcomingMatchInformationViewController: UIPageViewControllerDataSource {
...
extension UpcomingMatchInformationViewController: UIPageViewControllerDelegate {
...
extension UpcomingMatchInformationViewController: ScrollDelegate {
    
    private func setupScrollDelegates() {
        // 각 하위 뷰컨트롤러의 스크롤 델리게이트 설정
        if let matchSummaryVC = viewControllers[0] as? MatchSummaryViewController {
            matchSummaryVC.scrollDelegate = self
        }
        if let headToheadVC = viewControllers[1] as? HeadToHeadViewController {
